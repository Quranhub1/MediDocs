...
const Header = ({ user, userProfile, onLoginClick, onRegisterClick, onLogoutClick, onMenuClick, onAISearch, currentView, onViewChange }) => {
  ...
  return (
    <header className={`sticky top-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white dark:bg-dark-card shadow-lg' : 'bg-white dark:bg-dark-card'}`}>
      ...
      <div className="flex items-center space-x-3"><ThemeToggle /><button onClick={() => handleNavClick({ id: 'profile' })} className={`hidden sm:flex items-center gap-2 px-3 py-2 rounded-xl transition-all ${currentView === 'profile' ? 'bg-emerald-100 dark:bg-emerald-900/30' : 'bg-emerald-50 dark:bg-gray-700 hover:bg-emerald-100 dark:hover:bg-gray-600'}`} aria-label="Open my profile"><span className="w-8 h-8 rounded-full overflow-hidden bg-gradient-to-br from-emerald-500 to-teal-600 text-white flex items-center justify-center font-bold text-sm">{profilePhoto ? <img src={profilePhoto} alt={`${displayName} profile`} className="w-full h-full object-cover" /> : displayName.charAt(0).toUpperCase()}</span><span className="text-sm font-medium text-gray-700 dark:text-dark-text max-w-[120px] truncate">{displayName}</span></button><button id="logout-button" onClick={onLogoutClick} className="px-4 py-2 text-red-600 dark:text-red-400 font-medium hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg">Logout</button></div></div></div>
    </header>
  );
};
