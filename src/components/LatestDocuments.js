...
const LatestDocuments = ({ documents, user, userProfile, onViewChange }) => {
  ...
  const handleReadOnline = (doc) => {
    if (!isSubscriber) {
      ...
      if (limitReached && !hasViewed(doc.id)) {
  ...
  const remainingViews = Math.max(FREE_VIEW_LIMIT - viewedCount, 0);
      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-10">
          ...
          {!isSubscriber && (
            <p className="text-sm text-emerald-700 mt-2 font-medium">
              Free views remaining: {remainingViews} of {FREE_VIEW_LIMIT}
            </p>
          )}
        </div>
        ...
        <div className="relative" onMouseEnter={() => setIsPaused(true)} onMouseLeave={() => setIsPaused(false)}>
                  ...
                  <div key={doc.id || index} className="w-full shrink-0">
                    <div className="bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 border border-gray-100 overflow-hidden mx-auto max-w-3xl">
                      <div className="relative h-48 bg-gradient-to-br from-emerald-50 via-teal-50 to-cyan-50 overflow-hidden">
                        {getThumbnailUrl(doc) ? (
                          <img src={getThumbnailUrl(doc)} alt={doc.title} className="w-full h-full object-cover" loading="lazy" />
                        ) : (
                        ...
                        <div className="absolute top-3 right-3 flex gap-2">
                          <span className={`px-3 py-1 rounded-full text-xs font-medium ${doc.time === 'latest' ? 'bg-emerald-500 text-white' : doc.status === 'premium' ? 'bg-amber-500 text-white' : 'bg-gray-500 text-white'}`}>
                            {doc.time === 'latest' ? 'Latest' : doc.status === 'premium' ? 'Premium' : 'Free'}
                          </span>
                          {viewed && <span className="px-3 py-1 rounded-full text-xs font-medium bg-blue-500 text-white">Viewed</span>}
                        </div>
                      </div>
                      ...
                      <div className="p-5">
                        ...
                        <div className="flex flex-wrap gap-2 mb-4">
                          <span className="px-2 py-1 bg-emerald-50 text-emerald-700 text-xs rounded-lg">{doc.courseId?.toUpperCase()}</span>
                          <span className="px-2 py-1 bg-teal-50 text-teal-700 text-xs rounded-lg">{doc.semesterId?.toUpperCase()}</span>
                        </div>
                        <div className="flex flex-col gap-3">
                          <button onClick={() => handleReadOnline(doc)} disabled={!getDocumentUrl(doc)} className="w-full px-4 py-3.5 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white text-base font-semibold rounded-xl transition-all duration-200 shadow-md disabled:opacity-40 disabled:cursor-not-allowed">
                            Read Online
                          </button>
                          <button onClick={() => handleDownload(doc)} disabled={!getDocumentUrl(doc)} className="w-full px-4 py-3.5 bg-white border-2 border-emerald-500 text-emerald-600 hover:bg-emerald-50 text-base font-semibold rounded-xl transition-all duration-200 disabled:opacity-40 disabled:cursor-not-allowed">
                            Download
                          </button>
                        </div>
                      </div>
          ...
          {displayDocuments.length > 1 && (
            <>
              <button onClick={goToPrevious} className="absolute left-2 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/90 hover:bg-white rounded-full shadow-lg flex items-center justify-center transition-colors z-10" aria-label="Previous document">
              ...
              <button onClick={goToNext} className="absolute right-2 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/90 hover:bg-white rounded-full shadow-lg flex items-center justify-center transition-colors z-10" aria-label="Next document">
          ...
          <div className="flex justify-center gap-2 mt-6">
            {displayDocuments.map((_, index) => (
              <button key={index} onClick={() => setCurrentIndex(index)} className={`h-2 rounded-full transition-all duration-300 ${index === currentIndex ? 'w-6 bg-emerald-600' : 'w-2 bg-gray-300 hover:bg-gray-400'}`} aria-label={`Go to document ${index + 1}`} />
            ))}
          </div>
        </div>
      </div>

      <LimitReachedModal show={showLimitModal} onClose={() => setShowLimitModal(false)} onChoosePlan={handleChoosePlan} viewedCount={viewedCount} />
      <PaymentModal show={showPaymentModal} onClose={() => setShowPaymentModal(false)} selectedPlan={selectedPlan} onPaymentSuccess={handlePaymentSuccess} />
    </section>
  );
};
