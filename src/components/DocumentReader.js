...
const NON_EMBEDDABLE_HOSTS = [
  'mega.nz', 'icedrive.net', 'mediafire.com', 'drive.google.com',
  'dropbox.com', 'www.dropbox.com', '1drv.ms', 'app.box.com'
];
...
const DocumentReader = ({ document, onClose }) => {
  ...
  const decreaseFontSize = () => setFontSize((size) => Math.max(12, size - 1));
  const increaseFontSize = () => setFontSize((size) => Math.min(24, size + 1));

  return (
    <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center p-4" data-theme={theme}>
      <div ref={containerRef} className={`relative w-full max-w-5xl h-[90vh] bg-white dark:bg-dark-card rounded-2xl shadow-2xl overflow-hidden flex flex-col ${isFullscreen ? 'max-w-full h-full rounded-none' : ''}`}>
        <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-dark-border bg-gray-50 dark:bg-dark-bg gap-4">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <div className="flex items-center gap-1 bg-gray-100 dark:bg-gray-700 rounded-lg px-1">
              <button onClick={decreaseFontSize} className="px-2 py-1 text-gray-700 dark:text-dark-text" aria-label="Decrease text size">A−</button>
              <span className="text-xs text-gray-500 dark:text-dark-muted min-w-8 text-center">{fontSize}</span>
              <button onClick={increaseFontSize} className="px-2 py-1 text-gray-700 dark:text-dark-text" aria-label="Increase text size">A+</button>
            </div>
            <button onClick={() => downloadDocument(document)} className="px-3 py-1 bg-emerald-600 text-white rounded-lg text-sm hover:bg-emerald-700">Download</button>
            <button onClick={toggleFullscreen} className="px-3 py-1 bg-gray-200 dark:bg-gray-700 rounded-lg text-sm hover:bg-gray-300 dark:hover:bg-gray-600">{isFullscreen ? 'Exit Fullscreen' : 'Fullscreen'}</button>
