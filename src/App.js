...
function AppContent() {
  ...
  useEffect(() => {
    ...
    try { const installed = localStorage.getItem(PWA_INSTALLED_KEY) === 'true'; const promptSeen = localStorage.getItem(PWA_PROMPT_SHOWN_KEY) === 'true'; if (installed) setIsStandalone(true); setInstallPromptSeen(promptSeen || installed); } catch {}
    const handler = (event) => { try { if (standalone || localStorage.getItem(PWA_INSTALLED_KEY) === 'true' || localStorage.getItem(PWA_PROMPT_SHOWN_KEY) === 'true') return; event.preventDefault(); localStorage.setItem(PWA_PROMPT_SHOWN_KEY, 'true'); } catch { if (standalone) return; event.preventDefault(); } setInstallPromptSeen(true); setPwaInstallPrompt(event); setShowInstallHelp(false); };
    const installedHandler = () => { try { localStorage.setItem(PWA_INSTALLED_KEY, 'true'); localStorage.setItem(PWA_PROMPT_SHOWN_KEY, 'true'); } catch {} setPwaInstallPrompt(null); setShowInstallHelp(false); setInstallPromptSeen(true); setIsStandalone(true); };
  ...
  useEffect(() => { if (currentUser && checkLoginAnomaly) checkLoginAnomaly(currentUser.email, 'unknown', navigator.userAgent); }, [currentUser, checkLoginAnomaly]);
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-dark-bg pb-16 lg:pb-0 transition-colors duration-300 overflow-x-hidden">
        {isBanned && <div className="fixed inset-0 z-50 bg-red-50 dark:bg-red-900/20 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="banned-title"><div className="bg-white dark:bg-dark-card rounded-2xl shadow-2xl p-8 max-w-md text-center"><div className="text-6xl mb-4" aria-hidden="true">🚫</div><h2 id="banned-title" className="text-2xl font-bold text-gray-800 dark:text-dark-text mb-2">Account Banned</h2><p className="text-gray-600 dark:text-dark-muted mb-6">Your account has been banned. Please contact support for assistance.</p><button onClick={handleLogout} className="touch-target px-6 py-2 bg-red-500 text-white rounded-lg">Logout</button></div></div>}
        <div className="flex flex-col min-h-screen">
          ...
          <main id="main-content" tabIndex="-1" className="flex-grow md-page-transition"><Sidebar isOpen={isSidebarOpen} onClose={closeSidebar} onHomeClick={() => handleViewChange('home')} onCoursesClick={() => handleViewChange('courses')} onAboutClick={() => handleViewChange('about')} onContactClick={() => handleViewChange('contact')} onPrivacyClick={() => handleViewChange('privacy')} onAdminClick={() => handleViewChange('admin')} /><div className="w-full">{isAdminView ? <><AdminDashboard user={currentUser} onViewChange={handleViewChange} /><AdminUserRegistry /><SubscriptionManager /></> : <><MainContent view={currentView} user={currentUser} userProfile={userProfile} setView={handleViewChange} onLoginClick={() => setShowLoginModal(true)} onRegisterClick={() => setShowRegisterModal(true)} onContactClick={() => setShowContactModal(true)} onAIChatClick={() => setShowAIChatModal(true)} />{currentUser && currentView === 'home' && <UserSubscriptionPanel user={currentUser} userProfile={userProfile} onRenew={openRenewal} />}</>}</div></main>
          <BottomNav currentView={currentView} onViewChange={handleViewChange} user={currentUser} />
          <footer className="hidden lg:block bg-gradient-to-r from-emerald-600 to-teal-700 text-white py-8 px-4"><div className="max-w-7xl mx-auto"><div className="grid grid-cols-1 md:grid-cols-3 gap-8"><div><h4 className="font-bold text-lg mb-4">MediDocs Uganda</h4><p className="text-emerald-100 text-sm">Your trusted medical education platform for Ugandan students.</p></div><div><h4 className="font-bold text-lg mb-4">Quick Links</h4><ul className="space-y-2 text-sm text-emerald-100"><li><button onClick={() => handleViewChange('home')} className="touch-target hover:text-white">Home</button></li><li><button onClick={() => handleViewChange('courses')} className="touch-target hover:text-white">Courses</button></li><li><button onClick={() => handleViewChange('about')} className="touch-target hover:text-white">About Us</button></li><li><button onClick={() => handleViewChange('contact')} className="touch-target hover:text-white">Contact</button></li></ul></div><div><h4 className="font-bold text-lg mb-4">Contact</h4><ul className="space-y-2 text-sm text-emerald-100"><li>Email: kaigwaakram123@gmail.com</li><li>Phone: +256 749 846 848</li><li>Kampala, Uganda</li></ul></div></div><div className="mt-8 pt-4 border-t border-emerald-500 text-center text-sm text-emerald-200">2026 MediDocs Uganda. All rights reserved.</div></div></footer>
        </div>
        <LoginModal show={showLoginModal} onClose={() => setShowLoginModal(false)} onSwitchToRegister={() => { setShowLoginModal(false); setShowRegisterModal(true); }} /><RegisterModal show={showRegisterModal} onClose={() => setShowRegisterModal(false)} onSwitchToLogin={() => { setShowRegisterModal(false); setShowLoginModal(true); }} /><PaymentModal show={showPaymentModal} selectedPlan={selectedPaymentPlan} onClose={() => setShowPaymentModal(false)} onPaymentSuccess={async () => { await refreshUserProfile(); }} /><ContactModal show={showContactModal} onClose={() => setShowContactModal(false)} /><AIStudyAssistant show={showAIChatModal} onClose={() => setShowAIChatModal(false)} user={currentUser} userProfile={userProfile} />
        <AccessibilityControls />
        <button onClick={() => setShowAIChatModal(true)} className="touch-target fixed bottom-20 right-20 z-40 w-14 h-14 sm:w-16 sm:h-16 bg-gradient-to-r from-emerald-500 to-cyan-500 rounded-full shadow-2xl flex items-center justify-center hover:scale-110 transition-transform" style={{ animation: 'pulse 2s infinite' }} aria-label="Open AI study assistant"><svg className="w-7 h-7 sm:w-8 sm:h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2H5a2 2 0 002-2v10"></path></svg></button>
