...
function AppContent() {
  ...
  useEffect(() => {
    ...
    const handler = (event) => {
      try {
        if (standalone || localStorage.getItem(PWA_INSTALLED_KEY) === 'true') return;
        event.preventDefault();
      } catch { if (standalone) return; event.preventDefault(); }
    ...
    const installedHandler = () => {
      ...
      setPwaInstallPrompt(null); setShowInstallHelp(false); setInstallPromptSeen(true); setIsStandalone(true);
    };
  ...
  const installPWA = async () => {
    ...
    try {
      ...
      if (outcome === 'accepted') {
        try { localStorage.setItem(PWA_INSTALLED_KEY, 'true'); localStorage.setItem(PWA_PROMPT_SHOWN_KEY, 'true'); } catch {}
        setIsStandalone(true); setInstallPromptSeen(true);
      }
    } catch (error) { console.warn('PWA installation prompt failed:', error); setPwaInstallPrompt(null); }
  };
        ...
        {canShowInstall && (
          <div className="fixed top-4 left-1/2 -translate-x-1/2 w-[calc(100%-2rem)] sm:w-96 bg-white dark:bg-dark-card rounded-xl shadow-2xl p-4 z-[60] border border-gray-200 dark:border-dark-border" role="dialog" aria-label="Install MediDocs">
            ...
            {pwaInstallPrompt ? (
              ...
              <div className="mt-4 rounded-lg bg-gray-50 dark:bg-gray-800 p-3"><p className="text-xs font-semibold text-gray-800 dark:text-dark-text">Add MediDocs from your browser</p><p className="text-xs text-gray-600 dark:text-dark-muted mt-1">On iPhone/iPad, use your browser's Share menu and choose <strong>Add to Home Screen</strong>. On supported desktop or Android browsers, choose <strong>Install app</strong> or <strong>Add to Home screen</strong>.</p><button onClick={() => setShowInstallHelp(false)} className="mt-3 text-xs font-medium text-emerald-700 dark:text-emerald-400">Dismiss</button></div>
            )}
          </div>
        )}
        {!isStandalone && !installPromptSeen && !pwaInstallPrompt && !showInstallHelp && <button onClick={() => setShowInstallHelp(true)} className="fixed top-4 left-1/2 -translate-x-1/2 z-40 px-3 py-2 bg-white dark:bg-dark-card border border-gray-200 dark:border-dark-border rounded-lg shadow-lg text-xs font-semibold text-gray-700 dark:text-dark-text" aria-label="Show instructions to install MediDocs">Install MediDocs</button>}
